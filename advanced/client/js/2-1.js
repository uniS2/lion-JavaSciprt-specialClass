...
//# callstak 함수 호출 공간 / 이벤트 공간 ...
//? 함수 실행 시 callstack에 pow()라는 함수 저장 : 실행과 동시에 자기자신을 만나므로 일시정지 + 바깥 pow() 함수 callstack에 등록 ( pow(2,3) ) => 안쪽 pow(2,2) 실행됨과 동시에 안쪽 자기자신 함수를 만나 callstack에 저장 및 pow(2, 1) 실행
//? 함수가 안쪽 함수를 실행할 경우에는 바깥환경은 일시정지되고 callstack에 등록
//? break 조건이 없기 때문에 callstack이 계속 쌓여 에러가 남 => 재귀호출 할 경우에는 break를 설정해주어야함
//? stack: 후입선출 : 즉, pow(2,1) -> pow(2,2) -> pow(2,3)
//? 쓰는 이유? 성능상 문제가 있을 수 있어 근데 보기 편해! / 반복문은 읽기 힘들고 양이 많아진다는 불편함이 생김 이를 대신 할 수 있는게 재귀!

function pow(x, n) {
  // debugger 후 개발자도구-콘솔 에서 확인가능
  if (n === 1) return x;
  return x * pow(x, n - 1);
}
...
function sumSalaries(department) {
  // Array.isArray() -> typeof에서 object라고만 나오는 결과를 더 상세히
  if (Array.isArray(department)) {
    department.reduce((acc, current) => acc + current.salary, 0);
  } else {
    let sum = 0;
